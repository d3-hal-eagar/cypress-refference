Feature: Zip Code - Validation Criteria & Character Restriction
  ...
  Scenario: User enters invalid characters in Zip code multiple input errors
    When I have enter invalid characters "![]&@%?<>!$():;~^{}+=|_*ãü木✉" into valid input "1234" on the "zip" and I see validation error message "Please use numbers only in this field." on ex flow

  Scenario: User enters additional invalid characters in Zip code multiple input errors
    When I have enter invalid characters ".,#'\//\" " into valid input "1234" on the "zip" and I see validation error message "Please use numbers only in this field." on ex flow
